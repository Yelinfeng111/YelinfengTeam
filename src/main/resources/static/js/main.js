...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
    ...
    new Ajax.Request( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
        ...
        onSuccess: function( transport ) {
            ...
            {
                if ( $(elemContainer) )
                    $(elemContainer).update( '<div class="recommendation_ignored">?õ????????????????????????????</div>' );
            }
            else
            {
                $(divBtn).show();
...
function AddFreeLicense( subid, strDisplayName )
{
    ...
    $J.post( 'https://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
        ShowAlertDialog(
            strDisplayName,
            '%s ????????????????????????????? Steam ??????á?'.replace( /%s/, strDisplayName )
        ).done( function() {
                window.location.reload();
            });
        if ( typeof GDynamicStore != 'undefined' )
        ...
        if ( data && data.purchaseresultdetail == 9 )
            ShowAlertDialog( strDisplayName, '?ò?????????? Steam ??????á?' );
        else
            ShowAlertDialog( strDisplayName, '???ò??????????????????????????????' );
    }).always( function () {
...
{
    var Modal = ShowBlockingWaitDialog( '????????', '' );
...
{
    $J(elElement).find("*[data-background-images-url]").each(function(i, j){
        var $elTarget = $J(j);
        $elTarget.css({'background-image': 'url(' + $elTarget.data('background-images-url') + ')' });
    });

    $J(elElement).find("img[data-images-url]").each(function(i, j){
        var $elTarget = $J(j);
        $elTarget.attr('src', $elTarget.data('images-url') );
    });
